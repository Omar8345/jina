...
Apart from using the {ref}`Jina Client <client>`, the most common way of interacting way with your deployed Flow is via HTTP.
...
```bash
curl --request POST 'http://localhost:12345/search' --header 'Content-Type: application/json' -d '{"data": [{"text": "hello world"}]}'
```
